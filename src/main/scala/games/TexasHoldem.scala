...
// TODO: I would rename package games => gameType
object TexasHoldem {

  val listOfCombination: List[Combination] =
    List(StraightFlush, FourOfKind, FullHouse, Flush, Straight, ThreeOfKind, Pair, TwoPair)

  // TODO: Please extract all validation logic in separate object 'Validator'
  def validateBoard(board: String): Either[MyException, Board] = {
    ...
    // TODO: If you use such a method in several places, you don't need to replicate it!
    //       Just move it out of validateBoard and reuse for hands validation
...
//  }
  ...
  def getAnswer(board: String, hands: List[String]): String =
    validateBoard(board) match {
      ...
      case Right(boardCards) =>
        validateHands(hands) match {
          case Left(exception)  => exception.description
          case Right(handCards) =>
            createAnswerFromTupleList(
